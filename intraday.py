...
avg = df.groupby(pd.Grouper(key='Date-Time', axis=0, freq=(str(period)+'T'))).mean(numeric_only=True)
newDf['AveragePrice'] = avg.iloc[:,0]
newDf = newDf.fillna(0)
newDf = newDf.drop(newDf[newDf.TradeCount <= 0].index)
newDf['DollarVolumeTraded'] = newDf.TradeCount * newDf.AveragePrice
newDf['ShareVolumeTraded'] = df.groupby(pd.Grouper(key='Date-Time', axis=0, freq=(str(period)+'T'))).sum(numeric_only=True).iloc[:,1]
...
tzs = 'GMT+10'
jsonfile_new = {"data_source":"reuters", 
...
for i in range(len(newJSON)):
    ...
    jsonfile_new['events'].append({"time_object":{"timestamp":datetime_object, "duration":int(period), "duration_unit":"minutes","timezone":tzs},
                                   "event_type":'intraday aggregated stats',   
                                   "attribute":newJSON[i]})

# write
with open("jsonfile_new.json", "w") as write_file:
    json.dump(jsonfile_new, write_file, indent=4)