...
newDf['AReturn-VWAP'] = newDf.VWAP.pct_change()
newDf['LReturn-VWAP'] = np.log(newDf.VWAP) - np.log(newDf.VWAP.shift(1))
newDf = newDf.fillna('Not Applicable')
newDf = newDf[['VWAP', 'AReturn-VWAP', 'LReturn-VWAP']]
...
tzs = 'UTC+10'
jsonfile_TSV = {"data_source":"reuters", 
...
for i in range(len(newJSON)):
    ...
    jsonfile_TSV['events'].append({"time_object":{"timestamp":datetime_object, "duration":int(period), "duration_unit":"minutes","timezone":tzs},
                                   "event_type":'intraday aggregated stats',   
                                   "attribute":newJSON[i]})

# write
with open("jsonfile_TSV.json", "w") as write_file:
    json.dump(jsonfile_TSV, write_file, indent=4)